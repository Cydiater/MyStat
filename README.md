...
For development: `swift run`. To test Spotlight or Shortcuts, use the app bundle produced by `./build.sh`; the script also generates and packages the App Intents metadata required for discovery. The app has no Dock icon. Its menu provides larger charts with 3m / 15m / 1h ranges, iPhone sharing status, connected device names, Keep Awake, and Launch at Login.
