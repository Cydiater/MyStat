...
The current Mac source also exposes **Start Keep Awake**, **Stop Keep Awake**, and **Toggle Keep Awake** to Spotlight on macOS 26+ and Shortcuts on macOS 13+. Build and install the app, open it once, then press **⌘Space** and search for **Keep Awake** (or **MyStat**) to find the actions. They use the duration and **Keep Display On** preferences from the menu and launch MyStat if needed. Start restarts the selected timer if already active; Toggle turns the current session on or off. The menu and coffee indicator update immediately. On macOS 13–15, create a shortcut using one of MyStat's actions and run that named shortcut from Spotlight.
