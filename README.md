...
Mac companion 1.6.0 uses a native AppKit **NSMenu** for the dropdown. CPU, memory, and network charts remain visible, with **CPU Processes**, **Memory Processes**, **Network Apps**, **Details**, **iPhone**, and **Settings** opening real cascading submenus. macOS provides their material, selection highlights, arrows, checkmarks, placement, and keyboard navigation. The charts are custom views hosted by the native menu. **Keep Awake** is a single native submenu row showing Off or the remaining time. Its submenu contains the on/off control, duration presets, and **Keep Display On**, with standard checkmarks. Live readings and countdowns continue while a menu is open.
...
The current Mac source exposes **Start Keep Awake**, **Stop Keep Awake**, and **Toggle Keep Awake** in Shortcuts on macOS 13+. They use the duration and **Keep Display On** preferences from the menu and launch MyStat if needed. Start restarts the selected timer if already active; Toggle turns the current session on or off. The menu and coffee indicator update immediately. For Spotlight, import [Keep Awake.shortcut](Shortcuts/Keep%20Awake.shortcut), which calls Toggle and has **Show in Spotlight** enabled. If the result still does not appear, run `./scripts/install-spotlight-launcher.sh` to install **Keep Awake.app** in your Applications folder. It appears through Spotlight's normal application index and runs the same saved shortcut. See [Spotlight setup](Shortcuts/README.md). Direct App Intent discovery is also supported by macOS 26+, but did not surface these actions on the tested macOS 27 installation even though Shortcuts could run them.

The current Mac source adds an automatic **menu-bar smart stack**: one compact chart and value replaces the three adjacent graphs. Its width fits the visible text; Keep Awake adds space only while active. It features meaningful recent CPU, memory, or network changes, including drops and sustained high activity. A ten-second average is compared with a recent baseline; tiny idle fluctuations do not take over. Automatic switches have a 20-second minimum interval and require a clear score advantage. The selection stays fixed while the dropdown is open. Hover for the selection reason and full reading; use **Settings → Menu Bar** to pin CPU, Memory, or Network or return to **Automatic**. The dropdown retains all three charts and their process submenus. Network uses compact decimal B/s, K/s, M/s and larger units, with an arrow identifying the featured direction; its graph still shows download above and upload below the center line.
...
Updates use signed archives on GitHub Releases and a signed feed on GitHub Pages. No system stats or process data are attached to update checks. See [release and signing instructions](AppStore/mac-updates.md). Version 1.6.0 provides a signed, notarized archive in the feed: Check for Updates offers **Install Update**, then **Install and Relaunch**. Preferences are preserved; an active Keep Awake session ends on relaunch. Older 1.2.0–1.3.0 entries remain historical manual-download notices. The submitted iPhone build is unchanged by this Mac release.
...
```sh
./build.sh     # Universal arm64 + x86_64 app; uses the project's development certificate when available
open MyStat.app
```

Run `scripts/test-network.sh --live` for network CSV framing, byte-rate calculation, app grouping, resets and gaps, native rows, graph scaling, light/dark render artifacts, and real collector sampling, CPU usage and shutdown. Run `swift test` for core tests and `scripts/test-status-menu.sh` on macOS for native-menu regression checks, including process value badges, empty/stale readings, stable live updates, action validation, nested keyboard shortcuts, chart layout, Keep Awake submenu actions and checkmarks, saved durations, display-option deadline preservation, errors, expiry, and countdown updates during menu tracking. Run `scripts/test-keep-awake.sh` to verify session lifecycle, countdown ticks and boundaries, real macOS sleep assertions including timed expiry, Spotlight/Shortcuts intent handlers, and generated discovery metadata.

For development: `swift run`. To test Spotlight or Shortcuts, use the app bundle produced by `./build.sh`; the script generates the App Intents metadata and signs with an installed Apple Development certificate matching the team in `MyStat-macOS/project.yml`. You can select another certificate with `MYSTAT_SIGNING_IDENTITY='Apple Development: Your Name (…)' ./build.sh` (substitute the exact identity from `security find-identity -v -p codesigning`). An ad-hoc signature has no team ID: the actions may be listed in Shortcuts, but macOS rejects their connection to the app. The app has no Dock icon. Its menu provides larger charts with 3m / 15m / 1h ranges, iPhone sharing status, connected device names, Keep Awake, and Launch at Login.

Local `build.sh` outputs are not notarized. Without a matching development certificate or an explicit signing identity, the script falls back to ad-hoc signing and warns that Spotlight/Shortcuts execution is unavailable. Public downloads require Developer ID signing and notarization. For releases, prefer the Xcode workflow below. If you already have a local Developer ID Application identity and a notarytool keychain profile, `scripts/release-macos.sh` provides another route using `MYSTAT_SIGNING_IDENTITY` and `MYSTAT_NOTARY_PROFILE`; it signs, notarizes, packages, and generates the signed feed.
...
ditto -c -k --keepParent .build/mac-notarized/MyStat.app .build/MyStat-v1.6.0.zip
scripts/generate-appcast.sh .build/MyStat-v1.6.0.zip AppStore/releases/1.6.0.md
```
