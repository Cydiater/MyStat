...
final class StatsChartView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        DashboardStyle.label(title.uppercased(), in: NSRect(x: 20, y: bounds.height - 26, width: 120, height: 16),
                             size: 10, color: DashboardStyle.muted, weight: .semibold)
        if onProcessPress != nil {
            DashboardStyle.label("TOP PROCESSES  ↗", in: NSRect(x: bounds.width - 144, y: bounds.height - 26, width: 124, height: 16),
                                 size: 9, color: DashboardStyle.muted, alignment: .right)
        }
        DashboardStyle.label(values.last.map { String(format: "%.1f%%", $0) } ?? "—",
                             in: NSRect(x: 20, y: bounds.height - 56, width: 116, height: 30),
                             size: 24, color: color, weight: .medium, mono: true)
        ...
        for index in 0...tickCount {
            ...
            let line = NSBezierPath()
            line.move(to: NSPoint(x: x, y: plot.minY)); line.line(to: NSPoint(x: x, y: plot.maxY))
            DashboardStyle.grid.setStroke(); line.lineWidth = 0.5; line.stroke()
        ...
        let line = NSBezierPath()
        line.lineJoinStyle = .round; line.lineCapStyle = .round; line.lineWidth = 1.5
        ...
        color.withAlphaComponent(0.13).setFill(); area.fill()
